...
function get_keyword( $type = null ){
    ...
    if($type == 3){
        ...
        $list_keyword = get_post_meta($int_setting_ids['common'],'common_kw_3',true);
        $error_text = 'コンテンツでデフォルト';
    }elseif ($type == 2) {
        if( isset( $_GET[ 'contentoptkw' ] ) ) $url_value = $_GET[ 'contentoptkw' ];
        $list_keyword = get_post_meta($int_setting_ids['common'],'common_kw_2',true);
        $error_text = '薬剤師専門サイトを<br>使うだけだとブラック求人に…？';
    }else {
        if ( isset( $_GET[ 'kaiwa' ] ) ) $url_value = $_GET[ 'kaiwa' ];
        $list_keyword = get_post_meta($int_setting_ids['common'],'common_kw_1',true);
